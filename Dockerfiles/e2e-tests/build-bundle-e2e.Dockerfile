# Build the bundle for e2e testing, golang version shouldn't matter much here, but if in doubt, keep it up-to-date with main Dockerfile
...
RUN VERSION=$OPERATOR_VERSION make bundle-e2e
RUN VERSION=$OPERATOR_VERSION make bundle-e2e ODH_PLATFORM_TYPE=rhoai
